...
//	DeleEvent delegateEvent;
    ...
    protected Color Color
    {
        get
        {
            if (tweener.GetComponent<Text>())
                return tweener.GetComponent<Text>().color;
            else if (tweener.GetComponent<SpriteRenderer>())
            {
                return tweener.GetComponent<SpriteRenderer>().color;
            }
			else if (tweener.GetComponent<Image>())
            {
				return tweener.GetComponent<Image>().color;
            }
            else if (tweener.GetComponent<Renderer>() && tweener.GetComponent<Renderer>().material.HasProperty("_Color"))
            {
                return tweener.GetComponent<Renderer>().material.color;
            }
            else if (tweener.GetComponent<Light>())
            {
                return tweener.GetComponent<Light>().color;
            }
            else if (tweener.GetComponent<UnityEngine.UI.Image>())
                return tweener.GetComponent<UnityEngine.UI.Image>().color;
        ...
        {
            if (tweener.GetComponent<Text>())
                tweener.GetComponent<Text>().color = value;
            else if (tweener.GetComponent<SpriteRenderer>())
            {
                tweener.GetComponent<SpriteRenderer>().color = value;
            }
			else if (tweener.GetComponent<Image>())
            {
				tweener.GetComponent<Image>().color = value;

            }
            else if (tweener.GetComponent<Renderer>() && tweener.GetComponent<Renderer>().material.HasProperty("_Color"))
            {
                if (tweener.GetComponent<Renderer>().material.HasProperty("_Color"))
                    tweener.GetComponent<Renderer>().material.color = value;
            }
            else if (tweener.GetComponent<Light>())
            {
                tweener.GetComponent<Light>().color = value;
            }
...
public class TweenAlpha : Tween<Color>
{
	...
	public override void Init ()
	{
        ...
        if ( _start.a == -1) {
            ...
            for (int i=0; i<tweener.transform.childCount; i++) {
                ...
                if (go.activeSelf)
                    go.AddTween (new TweenAlpha (_end.a, _duration));
			}
		} else {
			for (int i=0; i<tweener.transform.childCount; i++) {
                GameObject go;
                go = tweener.transform.GetChild(i).gameObject;
                if (go.activeSelf)
                    go.AddTween (new TweenAlpha (_start.a, _end.a, _duration));
			}
...
public static class GameObjectExtension
{
	...
	public static void KillTween (this GameObject obj)
	{
		...
		for (int i = 0; i < tweener.transform.childCount; i++)
		{
			(tweener.transform.GetChild(i).gameObject).KillTween();
		}
	...
	public static void Call (this GameObject obj, float delay, Action<GameObject> action)
	{
		obj.AddTween(new TweenDelay(delay).EndEvent( ()=>{
			action(obj);
		}));
